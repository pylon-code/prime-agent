...
export function verifyGhAttestationResult(output, expectedSubjects, expectedInvocation) {
	...
	if (
		!expectedInvocation || expectedInvocation.repository !== PYLON_PUBLICATION_REPOSITORY ||
		!["push", "workflow_dispatch"].includes(expectedInvocation.event) ||
		!new RegExp("^[0-9a-f]{40}$").test(expectedInvocation.sourceSha ?? "") ||
		!/^\.github\/workflows\/[A-Za-z0-9._-]+\.yml$/.test(expectedInvocation.workflow ?? "") ||
		(expectedInvocation.workflowRunId !== undefined && !/^[1-9][0-9]*$/.test(expectedInvocation.workflowRunId))
	) throw new Error("Attestation policy needs one exact canonical workflow invocation.");
	...
	for (const entry of results) {
		...
		const invocation = new RegExp(`^https://github\\.com/${expectedInvocation.repository}/actions/runs/([1-9][0-9]*)/attempts/([1-9][0-9]*)$`).exec(invocationId ?? "");
		if (
			...
			!invocation || (expectedInvocation.workflowRunId !== undefined && invocation[1] !== expectedInvocation.workflowRunId)
		) throw new Error("Attestation SLSA invocation does not bind the exact workflow run and source.");
		verifiedAttempts.set(`${invocation[1]}:${invocation[2]}`, { runId: invocation[1], runAttempt: invocation[2] });
		...
		const hasRekor = Array.isArray(verification?.verifiedTimestamps) && verification.verifiedTimestamps.some(
	...
	return [...verifiedAttempts.values()].sort((left, right) => Number(left.runId) - Number(right.runId) || Number(left.runAttempt) - Number(right.runAttempt));
}
...
export function validatePreviewWorkflowRunEvidence({ attempts }, previewManifest, attestedAttempts) {
	...
	if (!Array.isArray(attestedAttempts) || attestedAttempts.length === 0 || !Array.isArray(attempts)) {
		throw new Error("Preview attestation has no exact workflow attempt evidence.");
	}
	const keys = attestedAttempts.map(({ runId, runAttempt }) => `${runId}:${runAttempt}`).sort();
	if (new Set(keys).size !== keys.length || attempts.length !== keys.length) {
		throw new Error("Preview workflow attempt evidence is duplicate, missing, or ambiguous.");
	}
	const encountered = new Set();
	for (const evidence of attempts) {
		const { run, suite, jobs } = evidence;
		const key = `${run?.id}:${run?.run_attempt}`;
		encountered.add(key);
		if (
			!keys.includes(key) || String(run.id) !== expected.workflowRunId || run.run_number !== previewManifest.sequence ||
			...
			!Number.isSafeInteger(run.run_attempt) || run.run_attempt < 1 || !run.check_suite_id ||
			!["in_progress", "completed"].includes(run.status)
		) throw new Error("Preview sequence does not match the exact signed canonical workflow attempt.");
		if (suite.app?.id !== 15368 || suite.head_sha !== expected.sourceSha || suite.id !== run.check_suite_id) {
			throw new Error("Preview workflow attempt is not owned by the GitHub Actions app on the exact source.");
		}
		if (!Array.isArray(jobs) || jobs.some((job) => job.run_attempt !== run.run_attempt)) {
			throw new Error("Preview workflow jobs are not from the exact signed attempt.");
		}
		const attesters = jobs.filter((job) => job.name === "Approve and attest six preview subjects");
		if (attesters.length !== 1 || attesters[0].status !== "completed" || attesters[0].conclusion !== "success") {
			throw new Error("Preview workflow attempt lacks its one successful directly approved attester job.");
		}
	}
	if (encountered.size !== keys.length || keys.some((key) => !encountered.has(key))) {
		throw new Error("Preview workflow attempt evidence does not cover every exact signed attempt.");
	}
	return expected;
}

export function verifyPreviewWorkflowRun(previewManifest, attestedAttempts) {
	const attempts = attestedAttempts.map(({ runId, runAttempt }) => {
		if (runId !== previewManifest.workflowRunId || !/^[1-9][0-9]*$/.test(runAttempt)) {
			throw new Error("Signed preview attempt does not belong to the canonical manifest workflow run.");
		}
		const run = ghJson(`repos/${PYLON_PUBLICATION_REPOSITORY}/actions/runs/${runId}/attempts/${runAttempt}`);
		const suite = ghJson(`repos/${PYLON_PUBLICATION_REPOSITORY}/check-suites/${run.check_suite_id}`);
		const jobPage = ghJson(`repos/${PYLON_PUBLICATION_REPOSITORY}/actions/runs/${runId}/attempts/${runAttempt}/jobs?per_page=100`);
		if (!Array.isArray(jobPage.jobs) || jobPage.total_count !== jobPage.jobs.length) {
			throw new Error("Exact signed workflow attempt jobs are truncated or malformed.");
		}
		return { run, suite, jobs: jobPage.jobs };
	});
	return validatePreviewWorkflowRunEvidence({ attempts }, previewManifest, attestedAttempts);
}
...
export function verifyPreviewAttestations({ artifactDir, sourceSha, sourceTree, historical = false }) {
	...
	for (const subject of verified.subjects) {
		for (const attempt of verifySubject(join(artifactDir, subject.name), subject, verified.subjects, sourceSha, invocation)) {
			attempts.set(`${attempt.runId}:${attempt.runAttempt}`, attempt);
		}
	}
	verifyPreviewWorkflowRun(verified.previewManifest, [...attempts.values()]);
	return verified;
}
