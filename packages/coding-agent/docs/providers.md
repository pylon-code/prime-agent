...
The file is created with `0600` permissions (user read/write only). For model providers, auth file credentials take priority over environment variables, except for Prime Inference, where `PRIME_API_KEY` takes priority. Trace sharing has its own resolution order below.
...
Trace credential resolution checks `PRIME_AGENT_TRACES_API_KEY`, the Agent trace credential, `PRIME_API_KEY`, then the Agent Prime Inference credential, in that order.
