...
export class RlmSpawnLedger {
	...
	private async siblingsUnlocked(
		...
		const parentByChild = new Map(
			topology.edges.map((edge) => [canonicalSessionPath(edge.child), canonicalSessionPath(edge.parent)]),
		);
	...
	private async familyUnlocked(aliveEdges?: RlmLedgerEdge[]): Promise<SessionInfo[]> {
		...
		let alive: RlmLedgerEdge[] =
			aliveEdges ??
			(await this.liveEdgesUnlocked([...this.replaySync().values()].filter((candidate) => !candidate.deleted)));
