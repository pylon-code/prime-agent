...
export class DaemonWorkerClient {
	...
	async authenticateWorker(
		...
		timeoutMs = 3000,
	): Promise<Extract<DaemonResponse, { success: true }> & { data: DaemonWorkerAuthenticationResult }> {
		...
		return { ...response, data: { ...(response.data as DaemonWorkerAuthenticationResult), workerIncarnation } };
	}
