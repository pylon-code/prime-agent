...
describe("daemon worker supervisor monitoring", () => {
	...
	describe("caller-owned internal catch-up contract", () => {
		function harness(owned = true) {
			const activeSessionId = "owned-catchup-active";
			const launchEnv = cloneCallerOwnedSessionLaunchEnv({ OWNER_CANARY: "synthetic-owner-context" }) as Record<
				string,
				string
			>;
			const recoveryConfig = { cwd: "/synthetic/owner", noExtensions: true };
			const summary = {
				id: activeSessionId,
				activeSessionId,
				sessionId: "owned-catchup-session",
			...
			const result = {
				activeSessionId,
				lastEventSequence: 1,
				lastEventCursor: { generation: "owned-catchup-generation", sequence: 1 },
				snapshot: { summary, state: { sessionId: summary.sessionId }, messages: [] },
			} as unknown as DaemonAttachResult;
			const worker = {
				descriptor: {
					workerId: "owned-catchup-worker",
					rootActiveSessionId: activeSessionId,
					ownerClientId: owned ? "owner-client" : undefined,
					callerOwnedEnvironmentContract: owned,
					lifecycle: "ready",
					createCommand: { type: "create" as const },
				},
				client: {},
				launchEnv: owned ? launchEnv : undefined,
				launchEnvMode: owned ? "replace" : undefined,
				exactEnvironmentAwaitingOwner: false,
				transientCreateCommand: owned
					? {
							type: "create",
							launchEnvMode: "replace",
							launchEnv,
							config: recoveryConfig,
							env: { HERDR_PANE_ID: "retained-pane" },
						}
					: undefined,
				snapshotCache: new Map([[activeSessionId, result]]),
				snapshotGenerations: new Map(),
			...
			const client = {
				...
				capabilities: new Set(owned ? ["caller_owned_session_environment_cleanup_v1"] : []),
				supportsExtensionUi: false,
				attachedActiveSessionIds: new Set([activeSessionId]),
			} as unknown as DaemonSocketClient;
			const frames: unknown[] = [];
			const findWorkerForClient = vi.fn(async () => ({ worker, summary }));
			const supervisor = Object.assign(Object.create(DaemonSupervisor.prototype), {
				workers: new Map([[worker.descriptor.workerId, worker]]),
				clients: new Set([client]),
				protocolClientIds: new WeakMap(),
				...
				findWorkerForClient,
				publicSummary: (_worker: unknown, value: SessionSummary) => value,
				syncWorkerExtensionUi: vi.fn(async () => {}),
				streamReconstructor: { seed: vi.fn() },
				write: (_client: unknown, frame: unknown) => {
					frames.push(frame);
					return true;
				},
				writeSnapshotRecord: async (_client: unknown, frame: unknown) => {
					frames.push(frame);
					return true;
				},
				log: vi.fn(),
			}) as {
				queueCatchup(client: DaemonSocketClient, id: string, purpose?: "resync" | "replacement"): void;
				drainClientCatchups(client: DaemonSocketClient): Promise<void>;
				catchupAttachCommand(
					client: DaemonSocketClient,
					id: string,
					epoch: number,
				...
				advanceAttachmentEpoch(client: DaemonSocketClient, id: string): number;
			};
			return { supervisor, worker, client, frames, findWorkerForClient, activeSessionId, launchEnv, recoveryConfig };
		}

		it.each(["resync", "replacement"] as const)(
			"reuses exact retained context through real attach validation for %s",
			async (purpose) => {
				const h = harness();
				const descriptor = structuredClone(h.worker.descriptor);
				h.supervisor.queueCatchup(h.client, h.activeSessionId, purpose);
				await h.supervisor.drainClientCatchups(h.client);
				expect(h.frames).toEqual([
					expect.objectContaining({ type: purpose === "resync" ? "session_resynced" : "session_replaced" }),
				]);
				expect(h.worker.launchEnv).toEqual(h.launchEnv);
				expect(h.worker.transientCreateCommand).toMatchObject({
					launchEnv: h.launchEnv,
					launchEnvMode: "replace",
					config: h.recoveryConfig,
					env: { HERDR_PANE_ID: "retained-pane" },
				});
				expect(h.worker.descriptor).toEqual(descriptor);
				expect(JSON.stringify(h.frames)).not.toContain("synthetic-owner-context");
				expect(JSON.stringify(h.worker.descriptor)).not.toContain("synthetic-owner-context");
			},
		);

		it.each(["wrong-owner", "missing-context", "awaiting-owner", "different-environment"] as const)(
			"refuses %s without reconstructing owner authority",
			async (failure) => {
				const h = harness();
				if (failure === "wrong-owner") h.client.id = "stranger";
				if (failure === "missing-context") h.worker.transientCreateCommand = undefined;
				if (failure === "awaiting-owner") {
					h.worker.launchEnv = undefined;
					h.worker.exactEnvironmentAwaitingOwner = true;
				}
				if (failure === "different-environment")
					h.worker.transientCreateCommand!.launchEnv = { OWNER_CANARY: "replacement" };
				const retained = structuredClone(h.worker.transientCreateCommand);
				h.supervisor.queueCatchup(h.client, h.activeSessionId);
				await h.supervisor.drainClientCatchups(h.client);
				expect(h.frames).toEqual([expect.objectContaining({ type: "session_snapshot_failed" })]);
				expect(h.findWorkerForClient).not.toHaveBeenCalled();
				expect(h.worker.transientCreateCommand).toEqual(retained);
				expect(JSON.stringify(h.frames)).not.toContain("synthetic-owner-context");
			},
		);

		it("rejects a stale attachment epoch and skips a detached queued client", async () => {
			const h = harness();
			h.supervisor.advanceAttachmentEpoch(h.client, h.activeSessionId);
			expect(() => h.supervisor.catchupAttachCommand(h.client, h.activeSessionId, 0)).toThrow("no longer current");
			h.supervisor.queueCatchup(h.client, h.activeSessionId);
			h.client.attachedActiveSessionIds.clear();
			await h.supervisor.drainClientCatchups(h.client);
			expect(h.frames).toEqual([]);
			expect(h.findWorkerForClient).not.toHaveBeenCalled();
		});

		it("fences a detach while the real attach lookup is pending", async () => {
			const h = harness();
			const entered = createDeferred<void>();
			const resume = createDeferred<void>();
			h.findWorkerForClient.mockImplementationOnce(async () => {
				entered.resolve();
				await resume.promise;
				return { worker: h.worker, summary: h.worker.summaries.get(h.activeSessionId)! };
			});
			h.supervisor.queueCatchup(h.client, h.activeSessionId);
			const draining = h.supervisor.drainClientCatchups(h.client);
			await entered.promise;
			h.client.attachedActiveSessionIds.clear();
			h.supervisor.advanceAttachmentEpoch(h.client, h.activeSessionId);
			resume.resolve();
			await draining;
			expect(h.client.attachedActiveSessionIds.size).toBe(0);
			expect(h.frames).toEqual([]);
		});

		it("retains legacy non-owned catch-up and external environment rebinding refusal", async () => {
			const legacy = harness(false);
			legacy.supervisor.queueCatchup(legacy.client, legacy.activeSessionId);
			await legacy.supervisor.drainClientCatchups(legacy.client);
			expect(legacy.frames).toEqual([expect.objectContaining({ type: "session_resynced" })]);
			const owned = harness();
			await expect(
				owned.supervisor.attachClient(owned.client, {
					type: "attach",
					activeSessionId: owned.activeSessionId,
					capabilities: ["caller_owned_session_environment_cleanup_v1"],
					launchEnvMode: "replace",
					launchEnv: { OWNER_CANARY: "replacement" },
					recoveryConfig: owned.recoveryConfig,
				}),
			).rejects.toThrow("does not match the established snapshot");
		});
	});
	...
	it("subscribes to worker updates with chunked snapshots", async () => {
		type SubscriptionWorker = {
			client: {
				requestWorker: (command: unknown) => Promise<{ success: boolean }>;
	...
	it("uses a full snapshot subscription for a worker without the new transfer capability", async () => {
		type SubscriptionWorker = {
			client: {
				requestWorker: (command: unknown) => Promise<{ success: boolean }>;
				supportsServerCapability?: (capability: string) => boolean;
	...
	it("publishes recovery holds only with the exact restorable queued action state", async () => {
		...
		const createRecoverySupervisor = (residentWorker = worker) =>
			Object.assign(Object.create(DaemonSupervisor.prototype), {
				workers: new Map([[residentWorker.descriptor.workerId, residentWorker]]),
		...
		try {
			...
			const secondSupervisor = createRecoverySupervisor(restartedWorker);
...
`,
	...
	it("skips catalog startup when recovery has no interrupted operations", async () => {
		...
		try {
			await expect(supervisor.recoverUncertainWorkerOperations(worker)).resolves.toEqual(new Map());
