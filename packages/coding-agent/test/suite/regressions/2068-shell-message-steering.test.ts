...
const completion = { completionId: "a".repeat(32), pid: 42, command: "npm test", exitCode: 0 };

function completeShell(harness: Harness, completionId = completion.completionId) {
	return (harness.session as unknown as KernelSession)._createKernelHostHandlers()["bash.completed"]!({
		...completion,
		completionId,
	});
}

function readShellResult(harness: Harness, command = completion.command, completionId = completion.completionId) {
...
describe("#2068 shell message steering", () => {
	...
	it("consumes shell steering at the next tool boundary without waiting for the run to become idle", async () => {
		...
		try {
			...
			await expect(completeShell(harness)).resolves.toEqual({ completionId: completion.completionId });
	...
	it("withdraws a queued shell notice once the kernel reads the result", async () => {
		...
		try {
			...
			expect(harness.session.getSteeringMessages()).toHaveLength(1);
			// PID reuse retains separate per-handle identities.
			await completeShell(harness, "b".repeat(32));
			expect(harness.session.getSteeringMessages()).toHaveLength(2);
			await readShellResult(harness);
			expect(harness.session.getSteeringMessages()).toHaveLength(1);
			await readShellResult(harness, completion.command, "b".repeat(32));
			expect(harness.session.getSteeringMessages()).toEqual([]);
		} finally {
			release.resolve();
			await original;
