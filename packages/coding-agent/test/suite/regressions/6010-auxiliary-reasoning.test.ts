...
import { createExtensionProviderHooks } from "../../../src/core/extension-provider-hooks.js";
...
describe("auxiliary reasoning settings", () => {
	it.each(cases)("completes refinement, review, and side questions: $name", async (testCase) => {
		...
		await startSideQuestion(
			{ agent: parent, createScopedProviderHooks: () => createExtensionProviderHooks(() => undefined) },
			"side",
			"What happened?",
			(event) => {
				events.push(event);
